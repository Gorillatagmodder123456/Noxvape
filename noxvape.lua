...
local function buildColorPickerRow(parent, layoutOrder, initial, onChanged, labelText)
    ...
    dslider.Name = "Slider"
    dslider.AnchorPoint = Vector2.new(0.5, 0.5)
    dslider.Position = UDim2.new(0.5, 0, 1 - value, 0)
    dslider.Size = UDim2.new(1, 6, 0, 4)
    dslider.BackgroundColor3 = Color3.new(1, 1, 1)
    dslider.BorderSizePixel = 0
    dslider.ZIndex = 25002
    dslider.Parent = darkness

    local cdisp = Instance.new("Frame")
    cdisp.Name = "ColorDisplay"
    cdisp.BackgroundColor3 = current
    cdisp.BorderSizePixel = 0
    cdisp.Position = UDim2.fromOffset(8, 166)
    cdisp.Size = UDim2.fromOffset(172, 12)
    cdisp.ZIndex = 25002
    cdisp.Parent = pickerFrame
    ...
    cpc = UserInputService.InputChanged:Connect(function(input)
        ...
        if not frame.Parent then
            if cpc then cpc:Disconnect() end
            return
        end
...
local function getFlagsFromFolder(folder)
    ...
    for _, child in ipairs(folder:GetChildren()) do
        ...
        if flags then
            table.insert(list, { name = child.Name, flags = flags, instance = child })
        end
    end
    return list
end
    ...
    for _, rawPath in ipairs(raw) do
        ...
        if name and not seen[name] then
            ...
            if not isfile(path) then
                ...
                if isfile(t1) then path = t1
                elseif isfile(t2) then path = t2 end
            end
...
local function createFastFlagWindow()
    ...
    local function refreshList()
        ...
        for i, entry in ipairs(entries) do
            ...
            loadBtn.Size = UDim2.fromOffset(56, 26)
    ...
    loadAllBtn.MouseButton1Click:Connect(function()
        ...
        for name, _ in pairs(selectedFlags) do
            for _, e in ipairs(getFlagsFromFolder(folder)) do
                if e.name == name then
                    total = total + applyFastFlags(e.flags)
                    break
                end
            end
        end
...
local function createProfilesWindow()
    ...
    local function makeProfileRow(entry, index)
        ...
        loadBtn.MouseButton1Click:Connect(function()
            ...
            if not data then
                notifyError("Failed to read profile: " .. name)
                return
            end
...
local function createSettingsWindow()
    ...
    local function addVolumeSlider(order)
        ...
        sb.Position = UDim2.fromOffset(0, 26)
...
local function init()
    ...
    sc.Name = "SettingsButtonContainer"
