...
local function ensureFastFlagsFolder()
    ...
    local ok2, c = pcall(function()
        local f = Instance.new("Folder"); f.Name = FFLAGS_FOLDER_NAME; f.Parent = workspace; return f
    end)
...
local function repairConfig()
    ...
    if type(config.menuColor) ~= "table" then config.menuColor = { r = 55/255, g = 150/255, b = 200/255 }
    else
    ...
    if type(config.textColor) ~= "table" then config.textColor = { r = 235/255, g = 240/255, b = 245/255 }
    else
    ...
    if type(config.rainbowPickers) ~= "table" then config.rainbowPickers = {} end
    if type(config.language) ~= "string" or not Languages[config.language] then config.language = "en" end
...
local function getSetting(cat, feat, key, default)
    ...
    if v == nil then config.settings[cat][feat][key] = colorToData(default); return default end
...
local function showTooltip(text)
    ...
    task.delay(0.08, function()
        ...
        lbl.FontFace = UIFont; lbl.TextSize = 16
        lbl.TextColor3 = Colors.Text; lbl.Text = text
        lbl.ZIndex = 60001; lbl.Parent = frame
        task.wait()
...
local function applyMenuColorToGui(newColor)
    ...
    for _, d in ipairs(screenGui:GetDescendants()) do
        if d:IsA("GuiObject") then
            ...
            if colorClose(c, oldAccent) then d.BackgroundColor3 = Colors.Accent
            elseif colorClose(c, oldOn) then d.BackgroundColor3 = Colors.ToggleOn
            elseif colorClose(c, oldOnHover) then d.BackgroundColor3 = Colors.ToggleOnHover end
        end
        if d:IsA("UIStroke") and colorClose(d.Color, oldAccent) then d.Color = Colors.Accent end
        if d:IsA("ScrollingFrame") and colorClose(d.ScrollBarImageColor3, oldAccent) then d.ScrollBarImageColor3 = Colors.Accent end
    ...
    if colorClose(Colors.Text, oldAccent) then
        ...
        config.textColor = { r = Colors.Accent.R, g = Colors.Accent.G, b = Colors.Accent.B }
        saveConfig()
    end
end
local function applyTextColorToGui(newColor)
    ...
    for _, d in ipairs(screenGui:GetDescendants()) do
        if d:IsA("TextLabel") or d:IsA("TextButton") or d:IsA("TextBox") then
            if colorClose(d.TextColor3, oldText) then d.TextColor3 = Colors.Text end
        end
    end
    if tooltip then
...
local _toggleListeners = {}
local function fireFeatureToggle(cat, name, state)
    for _, fn in ipairs(_toggleListeners) do pcall(fn, cat, name, state) end
end
...
local function clearConfigAndRestart()
    if canUseFiles() then
        pcall(function() if isfile(CONFIG_FILE) then delfile(CONFIG_FILE) end end)
    end
    createNotification(L("config_cleared"), "warning")
    task.wait(0.5)
    selfDestruct()
end
...
local function applyFastFlags(flags)
    ...
    for k, v in pairs(flags) do
        if pcall(function() fn(tostring(k), tostring(v)) end) then count += 1 end
    end
    return count, nil
end

local function buildColorPickerRow(parent, layoutOrder, initial, onChanged, labelText, pickerId, showSync)
    ...
    local wheelDragging, dDragging = false, false
    ...
    darkness.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then dDragging = true; updateDM() end end)
    ds.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then dDragging = true; updateDM() end end)
    local cpc
    ...
    rgbBtn.ZIndex = 3
    rgbBtn.Parent = frame
    local autoRGB, rgbConn = false, nil
    local function startRGB()
        ...
        if pickerId then config.rainbowPickers[pickerId] = true; saveConfig() end
    end
    local function stopRGB()
        autoRGB = false
        if rgbConn then rgbConn:Disconnect(); rgbConn = nil end
        rgbBtn.BackgroundColor3 = Colors.Action
        if pickerId then config.rainbowPickers[pickerId] = false; saveConfig() end
    end
    local function toggleRGB() if autoRGB then stopRGB() else startRGB() end end
    ...
    if pickerId and config.rainbowPickers[pickerId] == true then startRGB() end
    return frame, setFullColor
end

local function makeSearchableDropdown(parent, order, label, options, current, onSelect)
    ...
    optionsFrame.LayoutOrder = order + 0.5
    ...
    searchBox.FontFace = UIFont; searchBox.TextSize = 14
    ...
    local function refreshList(filter)
        ...
        for i, opt in ipairs(options) do
            if filter == "" or string.find(string.lower(tostring(opt)), filter, 1, true) then
                ...
                ob.FontFace = UIFont; ob.TextSize = 14
    ...
    dd.MouseLeave:Connect(function() if not optionsFrame.Visible then dd.BackgroundColor3 = Colors.Action end end)
...
local function createSettingsWindow()
    ...
    for i, cat in ipairs(cats) do
        ...
        b.ZIndex = 40004
        b.Parent = list
        b.MouseEnter:Connect(function() b.BackgroundColor3 = Colors.ActionHover end)
        b.MouseLeave:Connect(function() b.BackgroundColor3 = Colors.Action end)
...
local function addSettingsCheck(parent, labelText, getter, setter, order, tip)
    ...
    l.FontFace = UIFont; l.TextSize = 15
    l.TextColor3 = Colors.Text
    l.TextXAlignment = Enum.TextXAlignment.Left
    l.Text = labelText
    l.Parent = f
    if tip then addTooltip(l, tip) end
...
local function addSettingsTextRow(parent, labelText, getter, setter, order, ph)
    local f = Instance.new("Frame")
    f.BackgroundTransparency = 1
    f.Size = UDim2.new(1, 0, 0, 34)
    f.LayoutOrder = order
    ...
    l.FontFace = UIFont; l.TextSize = 14
    l.TextColor3 = Colors.Text
    l.TextXAlignment = Enum.TextXAlignment.Left
    l.Text = labelText
    l.Parent = f
...
local function addSettingsVolSlider(parent, order)
    local f = Instance.new("Frame")
    f.BackgroundTransparency = 1
    ...
    l.TextColor3 = Colors.Text
    l.TextXAlignment = Enum.TextXAlignment.Left
    l.Text = L("volume") .. " (" .. math.floor(config.soundVolume * 100) .. "%)"
    l.Parent = f
    ...
    knob.MouseButton1Down:Connect(function() dr = true end)
    s.InputBegan:Connect(function(input)
        if input.UserInputType ~= Enum.UserInputType.MouseButton1 then return end
        dr = true
        local mp = UserInputService:GetMouseLocation()
        upd((mp.X - s.AbsolutePosition.X) / s.AbsoluteSize.X)
    end)
    UserInputService.InputChanged:Connect(function(input)
        if not dr or input.UserInputType ~= Enum.UserInputType.MouseMovement then return end
...
local function addSettingsBtn(parent, labelText, onClick, order, primary)
    local b = Instance.new("TextButton")
    b.AutoButtonColor = false
    ...
    b.LayoutOrder = order
    b.Parent = parent
    b.MouseEnter:Connect(function() b.BackgroundColor3 = primary and Colors.ToggleOnHover or Colors.ActionHover end)
    b.MouseLeave:Connect(function() b.BackgroundColor3 = primary and Colors.Accent or Colors.Action end)
...
local function init()
    ...
    for ci, catDef in ipairs(_categories) do
        ...
        cl2.FontFace = UIFont; cl2.TextSize = 18
        ...
        for ii, item in ipairs(items) do
            ...
            nameLabel.FontFace = UIFont; nameLabel.TextSize = 17
            ...
            local function performToggle()
                ...
                if ok and result ~= false then
                    ...
                    refreshColor()
                    if currentState then notifyEnabled(itemName .. " " .. L("toggled_on"))
            ...
            kbl.FontFace = UIFont; kbl.TextSize = 14
            ...
            kbb.MouseButton1Click:Connect(function()
                ...
                if waitingForBind then
                    waitingForBind.button.Text = getKeybind(waitingForBind.category, waitingForBind.feature) or L("none")
                    waitingForBind.button.BackgroundColor3 = Colors.Action
                end
                waitingForBind = { category = catName, feature = itemName, button = kbb }
                kbb.Text = L("press_key")
            ...
            if type(extraSettings) == "table" then
                for si, setting in ipairs(extraSettings) do
                    ...
                    elseif st == "colorpicker" then
                        ...
                        end, sn, catName .. "/" .. itemName .. "/" .. sk, true)
                    elseif st == "dropdown" then
                        ...
                        local cur = getSetting(catName, itemName, sk, df)
                        makeSearchableDropdown(settingsFrame, order, sn, options, cur, function(opt)
    ...
    sbx.FontFace = UIFont; sbx.TextSize = 16
    sbx.TextColor3 = Colors.Text
    sbx.PlaceholderText = L("search_features")
    ...
    UserInputService.InputBegan:Connect(function(input, gp)
        ...
        if waitingForBind then
            if key == "Backspace" or key == "Escape" then
                ...
                waitingForBind.button.Text = L("none")
            else
                local used = false
...
function NoxLib.onFeatureToggle(fn)
    table.insert(_toggleListeners, fn)
    return {
        Disconnect = function()
            for i, f in ipairs(_toggleListeners) do
                if f == fn then table.remove(_toggleListeners, i); break end
            end
        end
    }
end
